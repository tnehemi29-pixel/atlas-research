...
import { claimReview } from '@/lib/services/researchReviewService';
import { mapWorkspaceServiceError } from '@/lib/workspace/errorMapping';

export const dynamic = 'force-dynamic';

/** POST /api/workspace/[id]/reviews/[reviewId]/claim */
export async function POST(_request: Request, { params }: { params: { id: string; reviewId: string } }) {
  ...
  try {
    const review = await claimReview(user.id, params.id, params.reviewId);
