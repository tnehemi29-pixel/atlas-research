...
import { searchFiling } from '@/lib/api/filings';
import { useDebouncedValue } from '@/lib/hooks/useDebouncedValue';

const DEBOUNCE_MS = 300;

interface FilingSearchBoxProps {
  filingId: string;
  onQueryChange: (query: string) => void;
}

export function FilingSearchBox({ filingId, onQueryChange }: FilingSearchBoxProps) {
  const [input, setInput] = useState('');
  const debounced = useDebouncedValue(input.trim(), DEBOUNCE_MS);
  const isQueryable = debounced.length >= 2;

  const { data: results = [], isFetching } = useQuery({
    queryKey: ['filing-search', filingId, debounced],
    queryFn: ({ signal }) => searchFiling(filingId, debounced, signal),
    enabled: isQueryable,
  ...
  return (
    <section className="mt-8">
      <h2 className="text-ink font-serif text-lg font-semibold">Search This Filing</h2>
      <input
        type="text"
        value={input}
        onChange={(event) => handleChange(event.target.value)}
        placeholder="Search for a term within this filing…"
          ...
          <ul className="mt-2 space-y-2">
            {results.map((result, index) => (
              <li key={index}>
                ...
                >
                  <div className="text-ink/40 text-[10px] font-medium uppercase tracking-wide">{result.sectionTitle}</div>
